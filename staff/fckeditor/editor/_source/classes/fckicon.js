...
FCKIcon.prototype.CreateIconElement = function( document )
{
	...
	if ( this.Position )		// It is using an icons strip image.
	{
		...
		{
			// <img border="0" class="TB_Button_Image" src="spacer.gif" style="background-position: 0px -16px;background-image: url(strip.gif);">
	...
	else					// It is using a single icon image.
	{
		if ( FCKBrowserInfo.IsIE )
		{
			// IE makes the button 1px higher if using the <img border="0"> directly, so we
