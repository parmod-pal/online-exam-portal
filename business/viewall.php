...
if($_SESSION['rim_userid'] !='')
{
	...
	$teamdet=select("select * from basicinfo where userid='".$_SESSION['rim_userid']."' and payment_id='".$payid."' order by id desc limit 1"); 			
	...
	$proddet=select("select * from prodinfo where userid='".$_SESSION['rim_userid']."' and payment_id='".$payid."' order by id desc limit 1"); 			
	...
	foreach($cate as $cat)
	{		
		$mdet=select("select * from manpowerdet where userid='".$_SESSION['rim_userid']."' and category='".$cat."' and payment_id='".$payid."' order by id desc limit 1"); 			
	...
	$invendet=select("select * from inventory where userid='".$_SESSION['rim_userid']."' and payment_id='".$payid."' order by id desc limit 1"); 			
	...
	$dcdet=select("select * from directcost where userid='".$_SESSION['rim_userid']."' and payment_id='".$payid."' order by id desc limit 1"); 			
	...
	$indcdet=select("select * from indirectcost where userid='".$_SESSION['rim_userid']."' and payment_id='".$payid."' order by id desc limit 1"); 			
	...
	$fincostdet=select("select * from 	finance_deposit_investment where userid='".$_SESSION['rim_userid']."' and payment_id='".$payid."' order by id desc limit 1"); 			
	...
	$pdsdet=select("select profit_margin, sales_discount from prod_sales_cost where userid='".$_SESSION['rim_userid']."' and payment_id='".$payid."' order by id desc limit 1"); 			
	...
	$salesdet=select("select * from salesparticular where userid='".$_SESSION['rim_userid']."' and payment_id='".$payid."' order by id desc limit 1"); 			
...
{ 
	?>
<div class="modal video-modal fade" id="loss" tabindex="-1" role="dialog" aria-labelledby="loss">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h3 class="text-center poptitle">Notification<button type="button" style="float:right;" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></h3>
			</div>
			<div class="modal-body">	
				<div class="alert-default instruc text-center">
					<div class="v_loss">
						<h2 class="error">SORRY!!!<br/>YOUR FIRM HAS INCURRED A LOSS</h2>
						<h3>IF YOU WANT TO PLAY THE GAME AGAIN, PLEASE START AFRESH</h3><br/>
						<a class="btn btn-primary" href="myprofile.php?act=g">START</a><br/><br/>
						<a class="btn btn-primary" href="common/logout.php">LOG-OUT</a>
					</div>
					<div class="v_profit">
						<h2 class="success">CONGRATULATIONS!!!<br/>YOU HAVE PROFITABLY RUN YOUR BUSINESS</h2>
						<h3>IF YOU WANT TO PLAY THE GAME AGAIN, PLEASE START AFRESH</h3><br/>
						<a class="btn btn-primary" href="myprofile.php?act=g">START</a><br/><br/>
						<a class="btn btn-primary" href="common/logout.php">LOG-OUT</a><br/><br>
						<p class="success">The "Certificate of Successful Completion" will be mailed to your email id.</p>
					</div>
				</div>
			</div>
		</div>
...
?>
<a href="#loss" id="start" class="btn btn-white" data-toggle="modal">&nbsp;</a>
...
$(function() { 
 ...
 setInterval(function(){ 
	var netprf=accounting.unformat($('#profitat').val());
	if(netprf > 0)
	{
		$('.v_loss').hide();
		$('.v_profit').show();
