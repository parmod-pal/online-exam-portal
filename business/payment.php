...
if (strcmp ($res, "VERIFIED") == 0) {
	...
	$isUniqueTxnId = false; 
	$result = select("SELECT * FROM payment WHERE txn_id = '$txn_id'");
	if(empty($result)) {
        $isUniqueTxnId = true;
	}	
	...
	$ins_det['payment_amount']=$payment_amount;
	$ins_det['payment_currency']=$payment_currency;
	$ins_det['txn_id']=$txn_id;
	$ins_det['dateoftrans']=$date;
	// check that receiver_email is your PayPal email
	// check that payment_amount/payment_currency are correct
	if($isPaymentCompleted && $isUniqueTxnId && (($payment_amount == "499" && $payment_currency == "INR") || ($payment_amount == "19.99" && $payment_currency == "USD"))) {
