...
if(isset($_REQUEST['login']))
{
	...
	{		
		...
		if(count($result)>0)
		{		
			if($user==$result['mailid'] && $password==$result['pass']){			
				$_SESSION['login'] = 1;
				$_SESSION['rim_userid']=$result['id'];				
				$_SESSION['currency']=$_POST['curncy'];
...
function load_user($user,$password){
    global $con;
    $select = array();
    $sql = "SELECT * FROM userdet WHERE emailid = '".$user."' AND password = '".$password."'";
     $data = mysqli_query($con,$sql);
         //var_dump($data);
   while($result=mysqli_fetch_assoc($data)){
        //var_dump($result);
	...
	if(empty($errors)){
		...
		if($chkemail==1)
		{
			...
			if($team_det_id > 0)
			{
				...
				$mail->Host       = "bh-in-3.webhostbox.net";      		// sets azillesoft as the SMTP server
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<form id="lgn" action="<?php echo $_SERVER['PHP_SELF'];?>" name="login" novalidate method="post">
					...
					<div class="form-group has-feedback">
						<select id="curncy" name="curncy" class="form-control" >
							<option value="">Select Version</option>
							<option value="rupee">Indian Version</option>
							<option value="dollar">US Version</option>
						</select>
					</div>
					<div class="row">
				...
				<form id="frgpwd" class="m-t" role="form" method="post" name="frgpwd" action="./" novalidate style="display:none;" >
					...
					<a class="btn btn-sm btn-white btn-primary" href="./">login</a>
