...
global $conn;
$con=$conn; $date=date('Y-m-d');$id1='';$errors=''; $name='';$usid='';$chk=0;$s=0; $nfrom='';$n=0; 
if(isset($_POST['6_letters_code']))
{
	if(empty($_SESSION['6_letters_code'] ) || strcasecmp($_SESSION['6_letters_code'], $_POST['6_letters_code']) != 0) { $errors .= "\n The captcha code does not match!"; }
		...
		$randnum=mt_rand(); $image="";$photo="";$sslc="";$ugc="";
		...
		if($photo !="") { 
			...
			if($res > 0) { 
				...
				if(($result1)>0) { 
					...
					{			
						$id1=$data['cf_id']; $email=$data['emailid']; $randno=$data['encrypt']; 
					} 
				}   
				
				$subject="Online Application For Diploma Courses";
				$msg="Click the below link to download the application <br/> <a href='https://www.rimsr.in/exemple01.php?napp=$id1&enc=$randno'>Download Application</a> <br/>";
				if($image !="") 
				{ 
				...
				/* $from = "enquiry@rimsr.in";
				$mailsent = mail("sakthivel@azillesoft.com","$subject","$msg","From: $to1\r\nReply-To: $from\r\nContent-type:text/html;");
				if($mailsent > 0) { 
					$s=1; 
				} */
			} 
...
{ 
?> 
	<script type="text/javascript"> alert ("<?php echo $ms;?>"); window.location="<?php echo $url;?>"; </script> 
<?php 
} 
function mss($ms)
 {
?> 
	<script type="text/javascript"> alert ("<?php echo $ms;?>"); </script> 
<?php
 }
?>
